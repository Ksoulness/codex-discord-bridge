...
关于如何切换后续模型、Queue 与 Steer（插队）的区别、撤回排队消息，以及状态灯为何可能延迟，请见[功能档案](docs/codex-discord-capability-archive.zh-CN.md)。

- 同步用户消息、过程说明、最终回答、命令、文件编辑和受支持的审批请求。
- 用频道状态灯显示任务实时状态：🟡 进行中/重连中、🔴 等待授权或错误、🟢 已完成/已停止、⚪ 已暂停监控。
- 将 Codex 的结构化计划进度同步到任务状态，包括当前步骤与总步骤。
- 在 Discord 中响应精确匹配的受支持审批和计划反馈请求。
- 使用 `/codex send` 从 Discord 继续同一个已映射 Codex 任务的对话；这是要求 Codex 修改项目的直接对话入口。
- 任务忙碌时可将消息 Queue 排队，按顺序作为下一回合发送；使用 `/codex retract` 撤回最新一条尚未发送的排队消息。
- 可 Steer 引导已确认正在运行的 Codex 回合以调整当前工作方向，不会新建独立任务；活动回合不明确或过期时会拒绝操作。
- 可选地允许一名已配置的 Discord 控制者发送普通文本消息。
- 在私有管理面板中选择项目和对话，可暂停、恢复或经确认后清理 Discord 镜像；并在 Discord 线程中查看受支持的子代理活动。
- 将受支持的 Discord 图片附件交给同一 Codex 任务：仅允许 Discord CDN、限制格式/数量/大小，并对本地图片缓存执行 7 天轮转。
- 使用 SQLite 保存必要的本地状态；限制回合保留、使过期控件失效、对常见密钥模式脱敏，并在状态不明确时拒绝操作。
