...
这些操作只对**当前 Discord 频道映射的同一个 Codex 任务**生效；不会中途替换已运行回合的模型，也不会把消息发送给别的任务。

| 目的 | Discord 操作 | Bridge 对 Codex 的实际动作 | 限制 |
| --- | --- | --- | --- |
| 切换后续模型 | 在任务频道执行 `/codex model`，私有菜单选择模型；如出现推理强度菜单，再选择强度。选择“跟随 Codex 默认模型”可清除频道偏好。 | 将模型和推理强度保存为该频道的本机偏好，下一次由 Discord 发起的**新回合**带上该偏好。 | 不会改变已开始回合；只能选择 Codex 当前返回的模型及其支持的推理强度。 |
| 不打断当前工作 | 用 `/codex send` 的 `queue` 模式，或返回消息中的 Queue 控件。 | 先写入本机 SQLite；当前回合结束后按 FIFO 顺序作为同一任务的下一条用户消息提交。 | 每任务最多 10 条待发消息；已提交给 Codex 的消息不能撤回。 |
| 立即改变当前方向（插队） | 用 `/codex send` 的 `steer` 模式，或返回消息中的 Steer 控件。 | 向已确认的活跃 Codex 回合发送 steering 消息，让 Codex 在当前回合调整方向。 | 不是新回合，也不是强制停止；活跃回合不明确、已过期或空闲时会拒绝。 |
| 撤回刚排队的消息 | `/codex retract`。 | 删除该任务最新一条 `pending` 本机队列记录。 | 不影响已发送、正在发送或更早的队列项。 |

建议：只是补充下一步工作时用 **Queue**；只有需要立刻改变当前回合方向时才用 **Steer**。Steer 依赖 Codex 当前回合、Desktop IPC 与本机事件状态；Bridge 无法确认活动回合时会拒绝，不会猜测并发送到错误任务。

### 状态灯为什么会有延迟

状态灯不是 Discord 在线指示器，而是本机 Bridge 根据 Codex 事件更新的任务状态：

1. Codex 先产生回合、计划、审批、完成、停止或错误事件，Bridge 才能得知新状态。
2. Bridge 先更新状态消息和本机状态库；频道名前缀会合并并串行更新，避免 Discord 高频重命名限流或旧状态覆盖新状态。
3. 因此状态文本可能先变，频道名前缀随后才变；连续变化最终以**最新已确认状态**为准。
4. 重连或重启后只恢复可确认的状态，避免把旧的“进行中”误显示为当前状态。

🟡 表示已观察到进行中或重连；🔴 表示等待授权或已观察到错误；🟢 表示已确认完成/停止；⚪ 表示监控暂停。灯比消息慢不代表 Discord 可以直接取消或控制本机任务。
